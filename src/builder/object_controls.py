...
class ObjectControls:
    ...
    def get_events(self, event):
        ...
        if event.type == pg.MOUSEBUTTONDOWN:
            # print(f"{self.game.state_stack[-1].manager.objects}")
    ...
    def handle_select_action(self):
        ...
        if hit is not None:
            ...
            #print(f"{hit_object=}") 
            self.game.state_stack[-1].manager.select_object(hit_object)
        else:
            self.game.state_stack[-1].manager.clear_selected_objects()
